...
-- Agregar restricciones de clave externa a las tablas
...
ALTER TABLE keepcoding.revision_vehiculo ADD CONSTRAINT fk_revision_vehiculo FOREIGN KEY (matricula) REFERENCES keepcoding.vehiculo(matricula);
ALTER TABLE keepcoding.revision_vehiculo ADD CONSTRAINT fk_revision_moneda FOREIGN KEY (id_moneda) REFERENCES keepcoding.moneda(id);


INSERT INTO keepcoding.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
SELECT grupo FROM keepcoding.coches GROUP BY grupo;

-- Insertar datos en la tabla marca
INSERT INTO keepcoding.marca (nombre, id_grupo)
SELECT DISTINCT coches.marca, grupo.id FROM keepcoding.coches coches
INNER JOIN keepcoding.grupo grupo ON coches.grupo = grupo.nombre;

-- Insertar datos en la tabla modelo
INSERT INTO keepcoding.modelo (nombre, id_marca)
SELECT DISTINCT coches.modelo, marca.id FROM keepcoding.coches coches
INNER JOIN keepcoding.grupo grupo ON coches.grupo = grupo.nombre
INNER JOIN keepcoding.marca marca ON coches.marca = marca.nombre;

-- Insertar datos en la tabla aseguradora
INSERT INTO keepcoding.aseguradora (nombre_aseguradora)
SELECT DISTINCT coches.aseguradora FROM keepcoding.coches coches;

-- Insertar datos en la tabla color
INSERT INTO keepcoding.color (nombre)
SELECT DISTINCT coches.color FROM keepcoding.coches coches;

-- Insertar datos en la tabla moneda
INSERT INTO keepcoding.moneda (nombre)
SELECT DISTINCT coches.moneda FROM keepcoding.coches coches;

-- Insertar datos en la tabla vehiculo
INSERT INTO keepcoding.vehiculo (matricula, id_modelo, id_color, fecha_compra, kms_totales)
SELECT DISTINCT coches.matricula, modelo.id, color.id, coches.fecha_compra, coches.kms_totales FROM keepcoding.coches coches
INNER JOIN keepcoding.grupo grupo ON coches.grupo = grupo.nombre
INNER JOIN keepcoding.marca marca ON coches.marca = marca.nombre
INNER JOIN keepcoding.modelo modelo ON coches.modelo = modelo.nombre
INNER JOIN keepcoding.color color ON coches.color = color.nombre;

-- Insertar datos en la tabla poliza
INSERT INTO keepcoding.poliza (id_aseguradora, fecha_alta_seguro, matricula, estatus)
SELECT aseguradora.id, coches.fecha_alta_seguro, coches.matricula, false FROM keepcoding.coches coches
INNER JOIN keepcoding.aseguradora aseguradora ON coches.aseguradora = aseguradora.nombre_aseguradora;

-- Establecer la poliza mas reciente como en vigor
UPDATE keepcoding.poliza p
SET estatus = true
FROM (
    SELECT matricula, MAX(fecha_alta_seguro) AS max_fecha_alta
    FROM keepcoding.poliza
    GROUP BY matricula
) pv
WHERE p.matricula = pv.matricula AND p.fecha_alta_seguro = pv.max_fecha_alta;

-- Insertar datos en la tabla revision_vehiculo
INSERT INTO keepcoding.revision_vehiculo (matricula, importe_revision, id_moneda, fecha_revision, kms_revision)
SELECT DISTINCT coches.matricula, coches.importe_revision, moneda.id, coches.fecha_revision, coches.kms_revision FROM keepcoding.coches coches
INNER JOIN keepcoding.moneda moneda ON coches.moneda = moneda.nombre;
...
SELECT
    ...
    vehiculo.kms_totales AS total_kilometros,
    aseguradora.nombre_aseguradora AS aseguradora,
    poliza.id AS poliza
FROM keepcoding.vehiculo vehiculo
INNER JOIN keepcoding.modelo modelo ON vehiculo.id_modelo = modelo.id
INNER JOIN keepcoding.marca marca ON modelo.id_marca = marca.id
INNER JOIN keepcoding.grupo grupo ON marca.id_grupo = grupo.id
INNER JOIN keepcoding.poliza poliza ON vehiculo.matricula = poliza.matricula AND poliza.estatus = true
INNER JOIN keepcoding.aseguradora aseguradora ON poliza.id_aseguradora = aseguradora.id
INNER JOIN keepcoding.color color ON vehiculo.id_color = color.id;
